...
def ground_truth_dist(grid_original, goal_val): # Goal could be a tuple of the goal coordinates of a unique value for the goal location
    ...
    grid = [[length[(i,j)] if (i,j) in length.keys() else -1 for j in range(n)] for i in range(m)]
...
#     G_raw = nx.grid_2d_graph(m, n)
#     G = nx.Graph()                                                                                                                                     
#     fedges = filter(lambda x: grid[x[0][0]][x[0][1]] == grid[x[1][0]][x[1][1]] != obstacle_val, G_raw.edges())
...
def custom_collate_fn(batch):
    ...
    goals_tensors = torch.stack([sample[0].goal_tensor for sample in batch])
    dist_tensors = torch.stack([sample[0].real_dists for sample in batch])
    paths = []
    for s in batch:
        sample = s[0]
    ...
    return maps.to(device), starts.to(device), goals.to(device), goals_tensors.to(device), dist_tensors.to(device), paths.to(device)
...
def main():
    ...
    dataset = MapDataset(r'C:\Users\meiri\OneDrive - Technion\Desktop\5th semester\CogRobProject\test_dataset', lazy=False, sample_num=50000)
    # rows = 3
    # cols = 4
